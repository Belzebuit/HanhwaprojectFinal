...
def train_lstm(train_df, test_df, features, target='Cases', window_size=7):
    ...
    best_lstm.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=best_params['lr']), loss='mse')
    ...
    print(f"[LSTM] Test MAE: {lstm_mae:.3f}, Test RMSE: {lstm_rmse:.3f}")
...
def train_gru(train_df, test_df, features, target='Cases', window_size=7):
    ...
    best_gru.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=best_params['lr']), loss='mse')
    ...
    gru_mae = mean_absolute_error(y_test_orig, y_pred)
    gru_rmse = np.sqrt(mean_squared_error(y_test_orig, y_pred))
    print(f"[GRU] Test MAE: {gru_mae:.3f}, Test RMSE: {gru_rmse:.3f}")
...
def train_prophet(train_df, test_df, features, target='Cases'):
    ...
    model_prophet = Prophet(
        ...
        daily_seasonality=False,  # 일별 데이터가 아니므로
    ...
    prophet_mae = mean_absolute_error(prophet_df_test['y'], prophet_pred)
...
print(f"[Stacking Ensemble] MAE: {stack_mae:.3f}, RMSE: {stack_rmse:.3f}")
...
def train_tcn(train_df, test_df, features, target='Cases', window_size=14):
    ...
    def objective(trial):
        ...
        for train_idx, val_idx in tscv.split(X_seq):
            ...
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr)
            model_cv.compile(optimizer=optimizer, loss="mse") # 컴파일
    ...
    tcn_mae = mean_absolute_error(y_test_orig, y_pred)
    tcn_rmse = np.sqrt(mean_squared_error(y_test_orig, y_pred))

    print(f"[TCN] Test MAE: {tcn_mae:.3f}, Test RMSE: {tcn_rmse:.3f}")
...
mae_list = [mean_absolute_error(test_df['Cases'], rf_pred_test),
            mean_absolute_error(test_df['Cases'].iloc[:len(sarimax_pred)], sarimax_pred),
            mean_absolute_error(test_df['Cases'].iloc[7:], lstm_pred), # LSTM, GRU는 window_size=7
            mean_absolute_error(test_df['Cases'].iloc[7:], gru_pred),
            mean_absolute_error(test_df['Cases'], prophet_pred),
            mean_absolute_error(test_df['Cases'].iloc[14:], tcn_pred), # TCN window_size=14
            stack_mae]
...
# MAE 비교 시각화
plt.figure(figsize=(10, 6))
plt.bar(model_names, mae_list, color='skyblue')
